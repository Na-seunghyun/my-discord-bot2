...
/**
 * Mouse/touch panning for the viewport.
 * - Drag empty map space with left mouse button, or press the wheel button.
...
function canStartLeftMousePan(e) {
  if (!(e.button === 0 && e.pointerType === 'mouse')) return false;
  const interactive = e.target.closest(
    '.block, button, input, select, textarea, a, .badge, .trash-zone, .sidebar, .toolbar',
  );
  return !interactive;
}
...
export function setupPan(expand) {
  ...
  function isInsideViewport(target) {
    return target === viewport || viewport.contains(target);
  }

  function startMousePanCandidate(e) {
    if (!isInsideViewport(e.target) || !canStartLeftMousePan(e)) return;
    ...
    document.addEventListener('mouseup', onMousePanEnd, { once: true });
  }
  ...
  function onMousePanMove(e) {
    ...
    e.preventDefault();
    viewport.scrollLeft = mousePan.startLeft - dx;
    viewport.scrollTop = mousePan.startTop - dy;
    expand();
  }
  ...
  viewport.addEventListener('mousedown', startMousePanCandidate, { capture: true });
   ...
   * - Desktop: left mouse on empty map space or middle-button mouse.
  ...
  const bind = (targetEl) =>
    targetEl.addEventListener('pointerdown', (e) => {
      ...
      const isLeftMouseOnMap = canStartLeftMousePan(e);
      const isTouchOneFinger = e.button === 0 && e.pointerType === 'touch';
      if (!(isLeftMouseOnMap || isMiddleMouse || isTouchOneFinger)) return;
