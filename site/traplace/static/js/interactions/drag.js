...
import { state, cellPx, blockByEl, toggleDistancePairBlock } from '../state.js';
...
function toggleBlockDistanceTarget(block) {
  if (!block) return;
    ...
    if (isTouch) {
      ...
      el.addEventListener(
        ...
        () => {
          if (timer) clearTimeout(timer);
          el.removeEventListener('pointermove', onMoveCheck);
          if (!movedEnoughToPan && !state.drag && !state.panning?.moved) {
            toggleBlockDistanceTarget(block);
          }
          window.__suppressContextMenu = false;
        },
        { once: true },
