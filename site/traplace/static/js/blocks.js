...
import { state, blockByEl, clearDistancePairBlock } from './state.js';
 ...
 */
function blockBoundsInCells(b) {
  const { cx, cy } = posToCell(b.left, b.top);
  const width = b.kind === 'custom' ? b.width || b.size : b.width || b.size;
  const height = b.kind === 'custom' ? b.height || b.size : b.height || b.size;
  return { left: cx, top: cy, right: cx + width, bottom: cy + height, width, height };
}
...
function overlapsImmutableBlock(block, rect) {
  return state.blocks.some((other) => {
    if (other === block || !other.immutable) return false;
    return rectsOverlap(rect, blockBoundsInCells(other));
  });
}
...
export function validateAllObjects() {
  for (const b of state.blocks) {
    ...
    if (needsValidation) {
      const rect = blockBoundsInCells(b);
      const { left: cx, top: cy, width, height } = rect;

      if (overlapsImmutableBlock(b, rect)) {
        invalid = true;
      }
...
export function updateBlockPosition(el, snappedLeft, snappedTop) {
  ...
  if (b) {
    b.left = snappedLeft;
    b.top = snappedTop;
    if (state.distancePairBlocks.includes(b)) clearDistancePairLine();
...
export function deleteBlock(el) {
  ...
  if (removed) {
    ...
    clearDistancePairBlock(removed);
