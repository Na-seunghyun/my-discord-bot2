...
function setupMinimapViewboxFallback(attempt = 0) {
  const box = document.querySelector('.minimap-viewbox');
  const canvas = document.querySelector('.minimap-canvas');
  if (!box || !canvas) {
    if (attempt < 20) setTimeout(() => setupMinimapViewboxFallback(attempt + 1), 50);
    return;
  }

  const update = () => {
    const canvasRect = canvas.getBoundingClientRect();
    const wrapRect = canvas.closest('.minimap')?.getBoundingClientRect();
    if (!wrapRect || !canvasRect.width || !canvasRect.height) return;

    const scale = Math.min(196 / viewport.scrollWidth, 138 / viewport.scrollHeight);
    const mapW = viewport.scrollWidth * scale;
    const mapH = viewport.scrollHeight * scale;
    const offX = (196 - mapW) / 2;
    const offY = (138 - mapH) / 2;
    const w = Math.min(mapW, Math.max(54, viewport.clientWidth * scale));
    const h = Math.min(mapH, Math.max(54, viewport.clientHeight * scale));
    const centerX = offX + (viewport.scrollLeft + viewport.clientWidth / 2) * scale;
    const centerY = offY + (viewport.scrollTop + viewport.clientHeight / 2) * scale;
    const x = Math.max(offX, Math.min(centerX - w / 2, offX + mapW - w));
    const y = Math.max(offY, Math.min(centerY - h / 2, offY + mapH - h));

    box.style.transform = 'none';
    box.style.left = `${canvasRect.left - wrapRect.left + x * (canvasRect.width / 196)}px`;
    box.style.top = `${canvasRect.top - wrapRect.top + y * (canvasRect.height / 138)}px`;
    box.style.width = `${w * (canvasRect.width / 196)}px`;
    box.style.height = `${h * (canvasRect.height / 138)}px`;
  };
