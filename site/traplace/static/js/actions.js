 ...
 * - Applies labels with distances to traps
...
export function setTitles() {
  ...
  if (btnPairDist) btnPairDist.title = t('ui.toolbar.pairDist');
}
...
function blockCenterInCells(b) {
  ...
  return { x: cx + (b.width || b.size) / 2, y: cy + (b.height || b.size) / 2 };
}

function blockRectInCells(b) {
  const { cx, cy } = posToCell(b.left, b.top);
  const width = b.width || b.size;
  const height = b.height || b.size;
...
function centerDistance(a, b) {
  const ac = blockCenterInCells(a);
  const bc = blockCenterInCells(b);
  return Math.hypot(ac.x - bc.x, ac.y - bc.y);
}

function pairDistanceCells(a, b) {
  if (a.kind === 'city' && b.kind === 'city') return centerDistance(a, b);
  return rectGapDistance(blockRectInCells(a), blockRectInCells(b));
}

function needsSpecialMarchFormula(block) {
  return block.kind === 'castle' || block.kind === 'turret';
}
...
function applyCityLabelsWithTrapDistance() {
  ...
  for (const city of cities) {
    ...
    setParenValues(labelEl, valuesStr, t('palette.city'));
  }
...
/**
 * Compute move time between the two selected objects and draw it on the map.
 */
function applySelectedPairDistance() {
  const selected = state.distancePairBlocks.filter((b) => state.blocks.includes(b));

  if (selected.length !== 2) {
    alert(t('alert.needTwoDistanceTargets'));
    return;
  }

  const [from, to] = selected;
  if (needsSpecialMarchFormula(from) || needsSpecialMarchFormula(to)) {
    alert(t('alert.specialMarchFormulaNeeded'));
    return;
  }
...
export function setupActions() {
  ...
  btnPairDist?.addEventListener('click', () => {
    applySelectedPairDistance();
  });
