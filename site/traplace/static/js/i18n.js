...
import { I18N_DATA } from './i18n-data.js?v=legend-nash-1';
/**
...
const BUILTIN_EN = {
  ...
  ui: {
    ...
    usage:
      ...
      '• Bottom <b>delete zone area</b>: drag an item here to delete.<br>' +
    ...
    contextMenu: {
      fontSize: 'Font Size',
      wordWrap: 'Word Wrap',
      distanceTarget: 'Select for pair distance',
    },
  },
  palette: {
    hq: 'HQ',
  ...
  alert: {
    noCities: 'No cities.',
    noTraps: 'No traps.',
    needTwoDistanceTargets: 'Select two objects first.',
    specialMarchFormulaNeeded:
      'Castle and turret march times use a different formula. Please provide an in-game time sample.',
 ...
 * 2) Current page/script relative defaults for sub-folder deploys
 * 3) Fallback to origin-based defaults:
 *    - {origin}/static/i18n
 *    - {origin}/i18n
 * @returns {string[]}
 */
function getBases() {
  ...
  const pageRelative = new URL('static/i18n', location.href).href;
  const script = document.currentScript || document.querySelector('script[src*="/js/i18n.js"], script[src*="js/i18n.js"]');
  const scriptRelative = script?.src ? new URL('../i18n', script.src).href : pageRelative;
  return [...new Set([pageRelative, scriptRelative, 'static/i18n', './static/i18n', `${origin}/static/i18n`, `${origin}/i18n`])];
}
...
function requestJson(url) {
  if (typeof fetch === 'function') {
    return fetch(url, { cache: 'default', mode: 'cors' }).then((r) => {
...
async function fetchJsonFromBases(lang) {
  const g = typeof window !== 'undefined' ? window : {};
  const inlineDict = I18N_DATA[lang] || (g.I18N_INLINE && g.I18N_INLINE[lang]);
  if (inlineDict) return inlineDict;

  const bases = getBases();
  for (const base of bases) {
    const u = `${String(base).replace(/\/$/, '')}/${lang}.json`;
    try {
