...
const PUBLIC_INDEX_URLS = [
  `${PUBLIC_BASE_URL}/`,
  `${PUBLIC_BASE_URL}/auto_redeem.html`,
...
${PUBLIC_INDEX_URLS.map((loc) => `  <url><loc>${loc}</loc></url>`).join("\n")}
</urlset>
`;
const PUBLIC_SITEMAP_TXT = `${PUBLIC_INDEX_URLS.join("\n")}\n`;
...
Sitemap: ${PUBLIC_BASE_URL}/sitemap.txt
`;
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET" && url.pathname === "/sitemap.xml") {
      return publicTextResponse(PUBLIC_SITEMAP_XML, "text/xml; charset=utf-8");
    }
    if (request.method === "GET" && url.pathname === "/sitemap.txt") {
      return publicTextResponse(PUBLIC_SITEMAP_TXT, "text/plain; charset=utf-8");
    }
